...
export default function ArticleCard({ article }: { article: Post<ArticleFrontMatter> }) {
	...
	return (<Link href={`/articles/${article.slug}`}>
		<article className="grid grid-cols-[110px_1fr] gap-6 items-start rounded-[18px] bg-surface shadow-line py-6 px-6.5 cursor-pointer transition-[background,transform] duration-200 ease hover:bg-surface2 hover:translate-x-1 max-[640px]:grid-cols-1 max-[640px]:gap-2.5 max-[640px]:py-5 max-[640px]:px-5">
