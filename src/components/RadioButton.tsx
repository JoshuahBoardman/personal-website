export default function({ index, name, on, handler }: { index: number, name: string, on: boolean, handler: () => void }) {
	...
	return (
		<button key={`filter-option - ${name} `} onClick={handler} type="button" data-pad={name} data-pad-on={on} className="cursor-pointer border-none bg-bg shadow-[inset_0_0_0_1px_var(--line)] text-text2 rounded-xl p-2.25 flex flex-col justify-between items-start max-[640px]:p-[7px] max-[640px]:rounded-[10px]">
