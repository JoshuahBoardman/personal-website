...
export default function SmallHomeArticleCard({ article }: { article?: Post<ArticleFrontMatter> }) {
	...
	if (!article) {
		return (
			<article className="flex flex-col justify-between gap-5 rounded-2xl bg-surface shadow-line p-5.5 min-h-47.5 min-w-0 w-full h-full">
	...
	return (
		<Link href={`/articles/${article.slug}`} className="min-w-0">
			<article className="flex flex-col justify-between gap-5 rounded-2xl bg-surface shadow-line p-5.5 min-h-47.5 cursor-pointer transition-[background,transform] duration-200 ease w-full h-full hover:bg-surface2 hover:-translate-y-0.75">
