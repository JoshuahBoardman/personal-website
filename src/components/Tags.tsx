...
function tagStyle(tag: string, postType: PostType): string {
	...
	return (
		<>
			{post.data.tags.slice(0, maxCount).map((tag: string) =>
				(<span key={`${post.slug}-${tag}`} className={`py-1 px-2.25 ${tagStyle(tag.toLowerCase(), postType)}`}>{tag}</span>)
			)}
			{/*TODO: Add a on hover component to view hidden tags*/}
			{displayHiddenTagNumber && (
				<span className={`py-1 px-2.25 ${tagStyle("", postType)}`}> +{amountOfHiddenTags}</span >
			)}
		</>
